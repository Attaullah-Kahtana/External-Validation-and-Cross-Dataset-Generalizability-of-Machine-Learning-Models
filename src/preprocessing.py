...
def preprocess_dataset(df, seed=42):
    ...
    # Fit outlier boundaries on TRAINING data only.
    bmi_lower, bmi_upper = calculate_iqr_bounds(
        train_df["bmi"]
    )

    # Apply the training-derived rule to the internal splits.
    train_df = train_df[
        train_df["bmi"].between(
            bmi_lower,
            bmi_upper,
            inclusive="both",
        )
    ...
    metadata = {
        ...
        "bmi_upper": bmi_upper,
        "seed": seed,
        "smote_applied_to": "training_split_only",
    }
